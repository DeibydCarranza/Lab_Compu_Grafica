...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Reporte 4", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTemp = model = glm::translate(model, glm::vec3(1.5f, 0.0f, 0.0f)); //Modeltemp es aux  
		...
		model = glm::translate(modelTemp, glm::vec3(1.5f, 0.0f, 0.0f)); //Funciona como checkpoint
		model = glm::rotate(model, glm::radians(codo), glm::vec3(0.0f, 1.0, 0.0f));//Codo  //Al hacer la rotación se fija el pivote
		...
		model = glm::translate(modelTemp, glm::vec3(0.45f, 0.0f, 0.0));
		model = glm::rotate(model, glm::radians(dedo2), glm::vec3(0.0f, 0.0, 1.0f));
		model = glm::translate(model, glm::vec3(0.5f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, 0.3f, 0.25f));
		color = glm::vec3(1.0f, 0.0f, 1.0f);
		...
		//Dedo pulgar
		model = glm::translate(modelTemp2, glm::vec3(0.25f, -0.1f, 0.375f));
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, -1.0, 0.0f));
		modelTemp = model = glm::translate(model, glm::vec3(0.3f, -0.2f, -0.025f));
...
void Inputs(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
		rot -= 0.07f;
	if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS && hombro < 60.0f)
		hombro += 0.07f;
