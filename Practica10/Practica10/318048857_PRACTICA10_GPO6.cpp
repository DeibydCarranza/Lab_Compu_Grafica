...
bool recorridoNuevo1 = false;
bool recorridoNuevo2 = false;
bool recorridoNuevo3 = false;
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Practoca 10 - Animacion Compleja", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, PosIni + glm::vec3(movKitX, 0, movKitZ));
		model = glm::rotate(model, glm::radians(rotKit), glm::vec3(0.0f, 1.0f, 0.0));
		model = glm::translate(model,glm::vec3(1.5f, 0.3f, 2.6f));
		model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::translate(model, glm::vec3(1.5f, 0.3f, -0.9f));
		model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		LLanta.Draw(lightingShader);
		...
		model = glm::translate(model, PosIni + glm::vec3(movKitX, 0, movKitZ));
		model = glm::rotate(model, glm::radians(rotKit), glm::vec3(0.0f, 1.0f, 0.0));
		model = glm::translate(model, glm::vec3(-1.5f, 0.5f, -2.9f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0));
		model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		LLanta.Draw(lightingShader);
...
void animacion()
{
	...
	if (circuito)
	{
		...
		if (recorridoNuevo1) {
			...
			if (movKitX > 90)
			{
				recorridoNuevo1 = false;
				recorridoNuevo2 = true;
			}
		...
		if (recorridoNuevo2) {
			...
			movKitX -= velocidad * deltaTime;
			movKitZ += velocidad * deltaTime;
			if (movKitX < 0 && movKitZ > 90 )
			{
				recorrido3 = true;
				recorridoNuevo2 = false;
			}
